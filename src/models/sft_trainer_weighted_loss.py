...
What this does:
  ...
  - Saves the best LoRA adapter checkpoint to checkpoints/sft_weighted/
...
# Derived from inverse frequency + manual tuning for critical classes
SEVERITY_TOKEN_WEIGHTS = {
    "P0": 5.0,   # rarest + most critical
    "P1": 3.0,   # rare + high severity
    "P2": 2.0,   # moderately rare
    "P3": 1.0,   # majority class — baseline
    "P4": 2.5,   # rare but often confused with P3
}
...
class WeightedSFTTrainer(SFTTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        # build per-token weight tensor (default 1.0 everywhere)
        weights = torch.ones(logits.shape[0], logits.shape[1], device=logits.device)

        # boost weight on severity token positions
        ...
        loss = (loss * shift_weights.view(-1)).mean()
...
def train(cfg: dict):
    ...
    save_training_samples(dataset, cfg.get("output_dir", "checkpoints/sft_weighted"))  # ← add this
    ...
    print("\nStarting SFT training with weighted loss …")
    trainer.train()
